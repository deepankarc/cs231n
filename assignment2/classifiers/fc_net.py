...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None):
    ...
    ############################################################################
    z1, afc1_cache = affine_forward(X,self.params['W1'],self.params['b1'])
    a1, relu_cache = relu_forward(z1)
    scores, afc2_cache = affine_forward(a1,self.params['W2'],self.params['b2'])
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
    ...
    # compute gradients wrt W2 and b2
    dx2, grads['W2'], grads['b2'] = affine_backward(delta_L,afc2_cache)
    
    # compute delta_l (eq.2)
    delta_l = relu_backward(dx2,relu_cache)
    
    # compute gradients wrt W1 and b1
    _, grads['W1'], grads['b1'] = affine_backward(delta_l,afc1_cache)
...
class FullyConnectedNet(object):
  ...
  def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
    ...
    ############################################################################
    # concatenate all net dims into one list
  ...
  def loss(self, X, y=None):
    ...
    for l in xrange(self.num_layers-1):
        # define keys for params dict
        W_key = 'W%d' % (l+1)
        b_key = 'b%d' % (l+1)
        gamma_key = 'gamma%d' % (l+1)
        beta_key = 'beta%d' % (l+1)
        
        # compute forward affine scores
    ...
    b_end = 'b%d' % (self.num_layers)
    scores, afc_cache[self.num_layers] = affine_forward(a[self.num_layers-1],
                                           self.params[W_end],self.params[b_end])
