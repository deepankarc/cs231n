...
class ConvolutionalNet(object):
    ...
    def loss(self, X, y=None):
        ...
        # if batchnorm is NOT used, evaluate this block
        if(use_bn == False):
        ...
        # if batchnorm is used, evaluate this block
        else:
            for l in xrange(total_layers-1,-1,-1):
                # perform backprop for affine layers
                if(l >= conv_layers and l == total_layers-1):                
                    ...
                    delta_l, grads['W%d' % (2*conv_layers+l-1)], grads['b%d' % (2*conv_layers+l-1)], \
                    grads['gamma%d' % (2*conv_layers+l-1)], \
                    grads['beta%d' % (2*conv_layers+l-1)] = affine_batchnorm_backward(delta_l, fc_cache[l])
                ...
                # perform backprop for convolutional layers
                    ...
                    delta_l, grads['W%d' % (2*l+2)], grads['b%d' % (2*l+2)], \
                    grads['gamma%d' % (2*l+2)], grads['beta%d' % (2*l+2)] = conv_spatialbn_relu_pool_backward(delta_l, conv_cache[2*l+2])
                    delta_l, grads['W%d' % (2*l+1)], grads['b%d' % (2*l+1)], \
                    grads['gamma%d' % (2*l+1)], grads['beta%d' % (2*l+1)] = conv_spatialbn_relu_backward(delta_l, conv_cache[2*l+1])
